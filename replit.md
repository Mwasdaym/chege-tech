...
### Admin Panel (/admin)
- Login: Email + Password + TOTP 2FA (Google Authenticator, Authy); supports multi-admin login
- **Forgot Password** - Email-based 6-digit reset code with HMAC-signed tokens, rate limiting, and max 5 attempts
...
- `server/admin-users.ts` - Multi-admin user management (stored in DB settings, bcrypt-hashed passwords)
- `server/supabase-backup.ts` - Supabase customer backup module (optional, configurable in admin settings)
